...
## 프로젝트 수행 결과
### 코드: class - synchronized, volatile, DCL
```
...
-> 별도 동기화 코드 필요 없음.
- 장점: 
	- 자동 스레드 안전성: 자바의 enum은 클래스 로딩 시점에 단 한 번만 인스턴스화됨. 별도 동기화 코드 필요 없음.
	- 직렬화 안전성: enum 사용하면 직렬화와 역직렬화 과정에서도 싱글톤 특성 유지됨. 별도 readResolve 메서드 구현 필요 없음.
...
위 코드에서 `flag`는 `volatile`로 선언되어 있어서 
